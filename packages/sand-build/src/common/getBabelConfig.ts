...
import { buildTypeEnum, moduleTypeEnum, extensions } from '../constant';
import { getBabelConfigOpts } from '../types';
 ...
 * @param {*} opts
 */
const getIsBrowser = (opts: browserOpts): boolean => {
  const {
    // 构建方式rollup | babel
    buildType,
    // 构建类型esm | cjs | umd
 ...
 * @param {*} opts
 */
function getBabelConfig(opts: getBabelConfigOpts): any {
  const {
    // 构建方式rollup | babel
    buildType,
    // 构建类型esm | cjs | umd
